...
foreach($default_tab_settings as $key=>$value){
 ...
 if($wpts_prev_post!=$post_id){$wpts_count=0;}
	...
	if(is_singular() or $wpts['enable_everywhere'] == '1') {
		...
		if($wpts['support']=='1'){
		  return '<div class="wordpress-post-tabs tabs_'.$post_id.'_'.( $wpts_count - 1 ).'_wrap wordpress-post-tabs-skin-'.$wpts['stylesheet'].'""><center>'.$tab_content.$support_link.'</center></div>'.$script;
		}
		else {
		  return '<div class="wordpress-post-tabs tabs_'.$post_id.'_'.( $wpts_count - 1 ).'_wrap wordpress-post-tabs-skin-'.$wpts['stylesheet'].'""><center>'.$tab_content.'</center></div>'.$script;
		}
	}
	else {
		return null;
