<?php
/*
Plugin Name: WooCommerce Product Filter (shared on wplocker.com)
...
*/
	...
	class WC_Prdctfltr {
		...
		function prdctfltr_scripts() {
			...
			wp_register_style( 'prdctfltr-main-css', self::$url_path .'/lib/css/prdctfltr.css', false, '4.0.4' );
			wp_enqueue_style( 'prdctfltr-main-css' );

			if ( !in_array( 'mcustomscroll', $curr_scripts ) ) {
				wp_register_style( 'prdctfltr-scrollbar-css', self::$url_path .'/lib/css/jquery.mCustomScrollbar.css', false, '4.0.4' );
				wp_enqueue_style( 'prdctfltr-scrollbar-css' );
				wp_register_script( 'prdctfltr-scrollbar-js', self::$url_path .'/lib/js/jquery.mCustomScrollbar.concat.min.js', array( 'jquery' ), '4.0.4', true );
				wp_enqueue_script( 'prdctfltr-scrollbar-js' );
			}

			if ( !in_array( 'isotope', $curr_scripts ) ) {
				wp_register_script( 'prdctfltr-isotope-js', self::$url_path .'/lib/js/isotope.js', array( 'jquery' ), '4.0.4', true );
				wp_enqueue_script( 'prdctfltr-isotope-js' );
			}

			if ( !in_array( 'ionrange', $curr_scripts ) ) {
				wp_register_style( 'prdctfltr-ionrange-css', self::$url_path .'/lib/css/ion.rangeSlider.css', false, '4.0.4' );
				wp_enqueue_style( 'prdctfltr-ionrange-css' );
				wp_register_script( 'prdctfltr-ionrange-js', self::$url_path .'/lib/js/ion.rangeSlider.min.js', array( 'jquery' ), '4.0.4', false );
				wp_enqueue_script( 'prdctfltr-ionrange-js' );
			}

			wp_register_script( 'prdctfltr-main-js', self::$url_path .'/lib/js/prdctfltr_main.js', array( 'jquery', 'hoverIntent' ), '4.0.4', true );
		...
		function prdctfltr_redirect() {
			...
			if ( is_post_type_archive( 'product' ) || is_tax( self::$settings['wc_settings']['product_taxonomies'] ) ) {
				...
				if ( isset( $_REQUEST['product_cat'] ) ) {
					...
					//added by djay to fix error on String type
